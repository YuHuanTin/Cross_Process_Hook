...
#include "ShellCode_Socket_TCP_X86.h"
...
std::vector<std::uint8_t> ShellCode_Socket_TCP_X86::makeShellCode(std::size_t ControlBlockAddr, std::size_t RegisterStorageAddr) {
    std::uint8_t endSign[]          = {0x90, 0x90};
    auto         funBeginAddAndSize = getFuncAddrAndSize((std::size_t) tcpX86, endSign, 2);
